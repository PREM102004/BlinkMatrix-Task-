...
export class CartService {
  ...
  getTotalQuantity(): number {
    return this.cartSubject.getValue().reduce((sum, product) => sum + product.quantity, 0);
  }

 
  getTotalPrice(): number {
    return this.cartSubject.getValue().reduce((sum, product) => sum + product.price * product.quantity, 0);
  }
