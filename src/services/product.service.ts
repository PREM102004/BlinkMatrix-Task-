...
export class ProductService {
  ...
  private productSubject = new BehaviorSubject<Products[]>(this.getProductsFromLocalStorage());
  ...
  private saveProductsInLocalStorage(products: Products[]): void {
    ...
    this.productSubject.next(products);  // Emit the updated products array
  }

  addNewProduct(product: Products): void {
    debugger
    const products = this.getProductsFromLocalStorage();
    product.id = new Date().getTime();  // Ensure unique ID generation
